...
function checkHeaders(url, options) {
  if (
    get(options, "headers.Authorization") &&
    get(options, "headers.Authorization") === "Bearer null"
  ) {
...
export const effectRequest = (url, options, meta) => {
  ...
  return timeout(
    fetch(`${api || defaultApi}${url}`, fetchOptions)
  ).then(res => {
    if (res.ok) {
...
export const logError = function* logError(
  message,
  extra = "",
  app = "inspector"
) {
